...
font_path = os.path.join(proj_path, "BlistaBraille.ttf")
...
def draw(img, boxes, labels, scores, draw_result=False):
    ...
    braile_font = ImageFont.truetype(font_path, 15)
...
def get_text_from_predictions(predictions, language):
    braille_to = braille_to_eng if language == "english" else braille_to_ru
    extracted_text = []
    extracted_braille = []
    for line in predictions.keys():
        for prediction in predictions[line]:
            braille_letter = braille[prediction["label"]]
            extracted_braille.append(braille_letter)
            extracted_text.append(braille_to.get(braille_letter, braille_letter))
        extracted_text.append("\n")
        extracted_braille.append("\n")
    return "".join(extracted_text), "".join(extracted_braille)
